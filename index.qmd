...
Our intial analysis will use the default parameters. The ranger package
in R implements a fast version of random forest. We are classifying the
Diabetes Binary field using all of the remaining fields in the
dataframe. We are also outputting the variable importance using gini
...
This shows that our estimated error rate is around 25.6%, for an
...
dataset was used for this algorithm. Stratified means that and equal
...
the questionaire and still have high accuracy. Here we have selected the
top 6 most important questions and re-run the classification to gauge
accuracy. It appears that with these 6 parameters we get an accuracy
within 1% of the accuracy we got using all parameters.
...
For prediction we want to split the data into train (70%) and test (30%)
sets.

Next the model is fit on the train set. Finally the model is used to
predict on the test set and the true value of the test set is compared
...
We were able to use the Random Forest method to classify the data into Diabetic and Non-Diabetic with approximately 74.4% accuracy (prediction error 25.6%).  Additionally, the random forest classifier will rank the importance measures of the variables to determine the most important variables contributing to the classification.  Using this information we were able to select the top variables for a reduced questionnaire of 6 questions compared to the original 20 questions.  The classifier the reduced number of questions is able to classify the data with approximately 73.5% accuracy (prediction error 26.5%).

We demonstrated hyperparameter tuning for Random Forest for the mtry parameter and manually tuned the number of trees.  We also demonstrated using random forest for prediction of unseen data and showed the method of gauging prediction accuracy comparing the true value with the predicted value on the unseen test data.  We hope this is a good introduction to the purpose and usage of the random forest method for those who are not familiar with it.
