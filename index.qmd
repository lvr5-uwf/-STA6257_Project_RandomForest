...
predicted based on the prediction by the majority of the trees [@Breiman2001]. The
...
some effect [@Dietterich2000]. The criteria for splitting is generally to minimize Gini
...
$$ where $p(y_i)$ is the probability that a sample belongs to class i, and
c is the number of classes. As the sum of the probability squared approaches 1,
...
do not contain the class label [@Breiman2001]. Because the error rate can be
approximated in this way, the data does not necessarily have to be split
into test and train datasets as in other machine learning models [@Breiman2001].
