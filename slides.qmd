...
-   Random forest improves on the decision tree method for
    ...
    a continuous value).
-   The method is attributed to Dr. Leo Brieman of the University of
...
-   Improvements to decision tree classifiers reduce bias and variance.
    ...
    -   Bootstrap aggregating (bagging) and AdaBoost
...
-   Random Forest is an ensemble method (uses several methods to solve a problem).
...
-   AdaBoost is "equivalent to fitting an additive logistic regression
    model by a stage-wise estimation procedure," and successful because
    of imparting diversity to the ensemble, but with a tradeoff in
    accuracy [@Rodriguez2006].
...
-   As the size of datasets increases (defined by the number of
    variables exceeding the number of observations), the performance of
    statistical methods decline and machine learning methods are
    preferred [@Verikas2011].
-   Random forests have been used in several such studies related to
    biology and medicine. In applications such as these, there is
    usually a relationship between response and predictor variables and
    sometimes there are strong correlations between predictors.
-   Random forests have helped with prediction, ranking of influential
    variables, and identifying variables with interactions.
-   Other studies have found good performance in areas such as
    predicting customer churn, fraud detection, identifying bacteria and
    fish, and identifying eye disease [@Verikas2011].

## Applications

-   Determining risk factors for diabetes, a disease affecting 425
    million adults where chronic high blood sugar has effects such as
    damaging blood vessels, organs, nerves, and other complications
    [@Lin2019].
...
-   The general methodology of the Random Forest model is to create a
    number of decision trees that use a random subset of features for
    the prediction.
-   For any given row, the data values are run through all of the
    decision trees that were created.
-   The value for the class label is predicted based on the prediction
    by the majority of the trees [@Breiman2001].
...
-   The criteria for splitting is generally to minimize Gini Impurity
    which is a measure of the homogeneity in the nodes.
...
-   Regardless of that, splitting into test and train datasets continues
    to be a best practice, especially if comparing to or using the
...
-   This dataset focuses on diabetes, a chronic health condition affecting millions of people worldwide that can cause serious complications and lower quality of life.
-   There is a need to predict an individual's risk of developing diabetes in the hopes that preventive measures can be taken to improve outcomes [@DiabetesDataset].
-   This dataset is a subset of data from the The Behavioral Risk Factor Surveillance System (BRFSS), a survey conducted by the CDC to examine Americans' responses in the areas of health behaviors and conditions.
    -   The diabetes dataset includes 21 feature variables and 70,692 survey responses.
    -   The response variable is whether or not an individual has diabetes (indicated by 0 for no or 1 for pre-diabetes or diabetes).
    -   This dataset has a 50-50 binary split, meaning the data is evenly split between having diabetes or not [@DiabetesDataset].
...
-   Our goal: Determine the level of predictive accuracy of the variables in the data set while using the random forest method.
-   Our steps taken:
    -   Analyze variable importance measures to find the most important questions in the Behavioral Risk Factor Surveillance System (BRFSS) questionnaire.
    -   Create a version of the BRFSS questionnaire with less questions
    -   Gauge accuracy of the reduced version of the BRFSS questionnaire, with the idea that a reduced version will increase future participation in the survey.
    -   Our initial random forest analysis will use default parameters.
    -   We will use the **ranger** package in R to implement a fast version of random forest.
    -   We classify the Diabetes Binary field using all of the remaining fields in the data set.
    -   We output the variable importance using Gini impurity as the importance measure.
...
-   Our estimated prediction error is around 25.6%, for an accuracy rate
    of around 74.4%.
...
-   The most important variables impacting Diabetes were found to be
    BMI, General Health, High Blood Pressure, and Age.
...
-   Using this value should yield the optimal accuracy rate.
-   The number of trees was tuned manually and it was found that about
    2000 trees is optimal.
...
-   The goal is to reduce the number of questions needed on the Behavioral Risk Factor Surveillance System (BRFSS) and maintain high accuracy.
-   We select the top 4 questions and run the classification to gauge accuracy.
...
```
-   This result shows a questionnaire with 4 questions still has 74.0% accuracy (prediction error 26.0%). Amazing!
...
-   True Positive (TP) is a sample classified as positive which is in
    fact positive, True Negative (TN) a negative which
    indeed is negative, False Positive (FP) a positive when it is in fact negative, and False Negative (FN) a negative when it is in fact positive.
...
-   This allows us a deeper understanding of the model, as we can easily see the model's accuracy as it relates to positive and negative classifications. 
-   In this case our model's accuracy is approximately 75% as illustrated in the pie chart above.

## Conclusion

-   We were able to use the Random Forest method to classify the data
    into Diabetic and Non-Diabetic with approximately 74.9% accuracy
    (prediction error 25.1%).
-   Additionally, we showed how to use the random forest classifier to
